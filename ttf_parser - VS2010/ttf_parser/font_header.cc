...
static void LongDateTimeToString(char *buf, size_t buf_size, LongDateTime time);

void FontHeader::LoadTable(TableRecordEntry *entry, ifstream &fin) {
...
void FontHeader::DumpInfo(XmlLogger &logger) const {
  ...
  logger.Println("<checkSumAdjustment value=\"0x%08x\"/>", checksum_adjustment_);
