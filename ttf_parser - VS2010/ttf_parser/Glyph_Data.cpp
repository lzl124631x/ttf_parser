...
bool is_simply_glyph(SHORT num_contours) {
  return num_contours > 0;
}
bool is_composite_glyph(SHORT num_contours) {
...
/*                             Glyph Data                               */
/************************************************************************/
void Glyph_Data::load_table(Table_Record_Entry *entry, ifstream &fin) {
  data = NULL;
...
Glyph *Glyph_Data::load_glyph(const GLYPH_ID glyph_index) {
  glyph.reset();
...
void Glyph_Data::dump_info(Xml_Logger &logger) const {
  logger.println("<glyf>");
