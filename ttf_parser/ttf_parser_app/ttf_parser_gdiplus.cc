...
void RenderGlyph(HDC hdc, HBITMAP bmp, Glyph *glyph, size_t width, size_t height, bool render_point) {
  ...
  FWord glyph_x_max = glyph->header().x_max();
  FWord glyph_y_max = glyph->header().y_max();
  FWord glyph_x_min = glyph->header().x_min();
  FWord glyph_y_min = glyph->header().y_min();
  ...
  matrix.Translate((float)-ttf_x_min, (float)ttf_y_max, MatrixOrderAppend); // translate with (-xMin, yMax)
  matrix.Scale(x_ratio, y_ratio, MatrixOrderAppend); // scale with (xRatio, yRatio)
  g.SetTransform(&matrix);
  // render bound
  // FIXME: ugly code!
  g.DrawLine(&Pen(Color(50, 0, 0, 0), 1), (float)glyph_x_min, (float)ttf_y_max, (float)glyph_x_min, (float)-(height / y_ratio - ttf_y_max));
  g.DrawLine(&Pen(Color(50, 0, 0, 0), 1), (float)glyph_x_max, (float)ttf_y_max, (float)glyph_x_max, (float)-(height / y_ratio - ttf_y_max));
  g.DrawLine(&Pen(Color(50, 0, 0, 0), 1), (float)ttf_x_min, (float)glyph_y_min, (float)width / x_ratio + ttf_x_min, (float)glyph_y_min);
  g.DrawLine(&Pen(Color(50, 0, 0, 0), 1), (float)ttf_x_min, (float)glyph_y_max, (float)width / x_ratio + ttf_x_min, (float)glyph_y_max);
  // render glyph
