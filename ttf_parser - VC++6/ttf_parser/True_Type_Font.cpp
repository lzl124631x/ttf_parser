...
namespace ttf_dll{
  ...
  bool True_Type_Font::load_path(string &path){
    ifstream fin(path.c_str(), ios::in | ios::binary); // FIXME: think twice about the parameter!
    if(!fin.is_open()){
...
#define SNTPRINTF(buf, buf_len, len, format, val)\
  (len) += _sntprintf((buf) + (len), (buf_len) - (len), (format), val)
  void True_Type_Font::glyph_info(Glyph *glyph, TCHAR *buf, size_t buf_len){
    ...
    int len = 0;
    SNTPRINTF(buf, buf_len, len, _T("xMin: %d\n"), glyph->header.x_min);
    SNTPRINTF(buf, buf_len, len, _T("xMax: %d\n"), glyph->header.x_max);
    SNTPRINTF(buf, buf_len, len, _T("yMin: %d\n"), glyph->header.y_min);
    SNTPRINTF(buf, buf_len, len, _T("yMax: %d\n"), glyph->header.y_max);
    SNTPRINTF(buf, buf_len, len, _T("numberOfContours: %d\n"), glyph->header.num_contours);
    SNTPRINTF(buf, buf_len, len, _T("leftSideBearing: %d\n"), g_ttf->hmtx.get_lsb(glyph->glyph_index));
    SNTPRINTF(buf, buf_len, len, _T("advanceWidth: %d\n"), g_ttf->hmtx.get_aw(glyph->glyph_index));
  }
}