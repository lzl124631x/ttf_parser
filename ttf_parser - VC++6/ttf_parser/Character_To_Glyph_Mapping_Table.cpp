...
/******************************* Character_To_Glyph_Index_Mapping_Table ***********************************/
  void Character_To_Glyph_Index_Mapping_Table::load_table(Table_Directory_Entry *entry, ifstream &fin){
    ...
    int i;
    for(i = 0; i < num_encoding_tables; ++i, ++record){
...
/******************************* Encoding_Table ***********************************/
  ...
  void Encoding_Table::dump_table_header(FILE *fp, size_t indent){
    ...
    IND_FP(fp, indent, "<length value=\"%u\"/>\n", length);
    IND_FP(fp, indent, "<language value=\"%u\"/>\n", language);
  }
...
/******************************* Trimmed_Table_Mapping ***********************************/
  ...
  void Character_To_Glyph_Index_Mapping_Table::dump_info(FILE *fp, size_t indent){
    IND_FP(fp, indent, "<cmap tableVersion=\"0x%08x\" numberOfEncodingTables=\"%d\">\n",
      table_version_number, num_encoding_tables);
    ++indent;
    int i;
    for(i = 0; i < num_encoding_tables; ++i){
      Encoding_Record *record = &encoding_records[i];
      IND_FP(fp, indent, "<encodingRecord>\n");
      ++indent;
      IND_FP(fp, indent, "<platformID value=\"%u\"/>\n", record->platform_id);
      IND_FP(fp, indent, "<encodingID value=\"%u\"/>\n", record->encoding_id);
      IND_FP(fp, indent, "<offset value=\"%lu\"/>\n", record->byte_offset);
      --indent;
      IND_FP(fp, indent, "</encodingRecord>\n");
    }
    for(i = 0; i < num_encoding_tables; ++i){
    ...
    --indent;
    IND_FP(fp, indent, "</cmap>\n");
  }

  void Segment_Mapping_To_Delta_Values::dump_info(FILE *fp, size_t indent){
    IND_FP(fp, indent, "<cmap_format_4>\n");
    ++indent;
    dump_table_header(fp, indent);
    IND_FP(fp, indent, "<segCountX2 value=\"%u\"/>\n", seg_countx2);
    IND_FP(fp, indent, "<searchRange value=\"%u\"/>\n", search_range);
    IND_FP(fp, indent, "<entrySelector value=\"%u\"/>\n", entry_selector);
    IND_FP(fp, indent, "<rangeShift value=\"%u\"/>\n", range_shift);

    IND_FP(fp, indent, "<endCount>\n");
    dump_array<USHORT>(fp, indent + 1, end_count, (seg_countx2 >> 1), "%8u");
    IND_FP(fp, indent, "</endCount>\n");

    IND_FP(fp, indent, "<reservedPad value=\"%u\"/>\n", reserved_pad);

    IND_FP(fp, indent, "<startCount>\n");
    dump_array<USHORT>(fp, indent + 1, start_count, (seg_countx2 >> 1), "%8u");
    IND_FP(fp, indent, "</startCount>\n");

    IND_FP(fp, indent, "<idDelta>\n");
    dump_array<SHORT>(fp, indent + 1, id_delta, (seg_countx2 >> 1), "%8d");
    IND_FP(fp, indent, "</idDelta>\n");

    IND_FP(fp, indent, "<idRangeOffset>\n");
    dump_array<USHORT>(fp, indent + 1, id_range_offset, (seg_countx2 >> 1), "%8u");
    IND_FP(fp, indent, "</idRangeOffset>\n");

    IND_FP(fp, indent, "<glyphIdArray>\n");
  ...
  void Byte_Encoding_Table::dump_info(FILE *fp, size_t indent){
    IND_FP(fp, indent, "<cmap_format_0>\n");
    ++indent;
    dump_table_header(fp, indent);
    IND_FP(fp, indent, "<glyphIdArray>\n");
  ...
  void High_Byte_Mapping_Through_Table::dump_info(FILE *fp, size_t indent){
    IND_FP(fp, indent, "<cmap_format_2>\n");
    ++indent;
    dump_table_header(fp, indent);
  ...
  void Trimmed_Table_Mapping::dump_info(FILE *fp, size_t indent){
    IND_FP(fp, indent, "<cmap_format_6>\n");
    ++indent;
    dump_table_header(fp, indent);
    IND_FP(fp, indent, "<firstCode value=\"%u\"/>\n", first_code);
    IND_FP(fp, indent, "<entryCount value=\"%u\"/>\n", entry_count);
