...
namespace ttf_dll{
  ...
  void Index_To_Location::dump_info(FILE *fp, size_t indent){
    IND_FP(fp, indent, "<loca format=\"%s\">\n", loca_format ? "ULONG" : "USHORT");
    ++indent;
    IND_FP(fp, indent, "<offsets>\n");
    ++indent;
    if(loca_format){
      dump_array<ULONG>(fp, indent, offsets, num_glyphs, "%8u");
    }else{
      IND_FP(fp, indent, "<!-- ATTENTION: The SHORT version 'loca' stores the actual local offset divided by 2!"
